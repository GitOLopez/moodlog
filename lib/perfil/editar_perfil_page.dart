...
class EditarPerfilPage extends StatefulWidget {
  ...
  const EditarPerfilPage({Key? key, required this.datosActuales}) : super(key: key);
...
class _EditarPerfilPageState extends State<EditarPerfilPage> {
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 12),
              _campoTexto('Contacto emergencia (nombre)', _nombreContactoCtrl, obligatorio: false),
              const SizedBox(height: 12),
              TextFormField(
                controller: _contactoEmergenciaCtrl,
                keyboardType: TextInputType.number,
                decoration: const InputDecoration(labelText: 'Teléfono emergencia'),
              ),
              ...
              ElevatedButton(
                onPressed: () async {
                  if (_formKey.currentState!.validate()) {
                    ...
                    await DatabaseHelper().updateUser(datosActualizados);
                    if (!mounted) return;
  ...
  Widget _campoTexto(String label, TextEditingController controller, {bool obligatorio = true}) {
    return Padding(
